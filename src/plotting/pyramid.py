...
data = pd.read_csv("./pyramid_data.csv")
sns.set_style("ticks")
...
bar_plot = sns.barplot(x="ICU male", y="Bin", color=color_ICU, alpha=0.6, label="ICU", data=data,
                       order=bins_order, lw=0.4)

bar_plot = sns.barplot(x="ICU female", y="Bin", color=color_ICU, alpha=0.6, data=data,
                       order=bins_order, lw=0.4)
